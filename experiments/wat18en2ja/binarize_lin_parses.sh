...
tgt=ja
PROJ=$HOME/pascal
TEXT=$PROJ/data/wat18jaen/corpus
TAGS=$PROJ/data/wat18jaen/lin_parses
OUT=$PROJ/data/wat18jaen/corpus_lin_parses/${src}2${tgt}
OUTPUT=$OUT/wat18${src}2${tgt}
BPE_DIR="$PROJ/tools/subword-nmt"

# Combine text and parse files
mkdir -p $OUT
for lang in $src $tgt; do
  cp $TEXT/dev.tok.tok.$lang $OUT/valid.$lang
...
python preprocess.py \
	--source-lang $src \
	--target-lang $tgt \
	--trainpref $OUT/train.rand.bpe.32000 \
	--validpref $OUT/valid.bpe.32000 \
	--testpref $OUT/test.bpe.32000 \
	--destdir $OUTPUT \
	--workers 16 \
        --joined-dictionary
